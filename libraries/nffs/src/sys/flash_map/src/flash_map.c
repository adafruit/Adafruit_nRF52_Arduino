...
// Adafruit: flash map with only NFFS area
const struct flash_area adafruit_flash_map[] =
{
    /* FLASH_AREA_NFFS */
    {
        .fa_id        = FLASH_AREA_NFFS,
        .fa_device_id = 0,
        .fa_off       = NFFS_START_ADDR,
        .fa_size      = NFFS_SECTOR_NUM*4096, /* 28 kB */
    }
};

const struct flash_area *flash_map = adafruit_flash_map;
int flash_map_entries = sizeof(adafruit_flash_map) / sizeof (adafruit_flash_map[0]);
